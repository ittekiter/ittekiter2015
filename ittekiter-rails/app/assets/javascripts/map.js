	 ...
	 function searchRoot() {
	 	...
	 	it.directionsService.route(request, function(result, status) {
	 		if (status == google.maps.DirectionsStatus.OK) {
	 			var sendData = {
	 		/*位置関連情報をpostする際はオブジェクトをJSON.stringifyで変換しないと送れないらしいよ　byいしい*/
	 					origin: JSON.stringify(it.searchData.from.geometry.location),
	 					destination: JSON.stringify(it.searchData.to.geometry.location),
